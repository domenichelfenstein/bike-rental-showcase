...
module TaskBuilder =
    ...
    let unwrapException (agg: AggregateException) =
        ...
        if inners.Count = 1 then inners.[0] else agg :> Exception
    ...
    let rec combine (step: Step<unit>) (continuation: unit -> Step<'b>) =
        ...
        | ReturnFrom t -> Await(t.GetAwaiter(), continuation)
        | Await(awaitable, next) -> Await(awaitable, (fun () -> combine (next ()) continuation))
        ...
        if cond () then
            ...
            let rec repeat () =
                if cond () then
                    ...
                    | Await(awaitable, next) -> Await(awaitable, (fun () -> combine (next ()) repeat))
                else
                    zero
    ...
    let rec tryFinally (step: unit -> Step<'a>) fin =
        ...
        | Await(awaitable, next) -> Await(awaitable, (fun () -> tryFinally next fin))
    ...
    type BindS = Priority1
        with
            ...
            static member (>>=)(Priority1, task: 'a Task) =
                fun (k: 'a -> 'b Step) -> bindTask task k: 'b Step

            static member (>>=)(Priority1, a: 'a Async) =
                fun (k: 'a -> 'b Step) -> bindTask (Async.StartAsTask a) k: 'b Step

    type ReturnFromS = Priority1
        with

            static member inline ($)(Priority1, taskLike) = Binder<_>.GenericAwait(taskLike, ret)

            static member ($)(Priority1, a: 'a Async) =
                bindTask (Async.StartAsTask a) ret: Step<'a>

    type BindI = Priority1
        with
    ...
    type ReturnFromI = Priority1
        with

            static member inline ($)(_: Priority2, taskLike) = Binder<_>.GenericAwait(taskLike, ret)

            static member inline ($)(Priority1, configurableTaskLike) =
                Binder<_>.GenericAwaitConfigureFalse(configurableTaskLike, ret)
    ...
    type TaskBuilder() =
        ...
        member inline __.Bind(task: 'a Task, continuation: 'a -> 'b Step) : 'b Step = bindTask task continuation
    ...
    module HigherPriorityBinds =
        ...
        type TaskBuilder.ContextInsensitiveTaskBuilder with
            ...
            member inline this.ReturnFrom(configurableTaskLike) =
                TaskBuilder.Binder<_>
                    .GenericAwaitConfigureFalse(configurableTaskLike, TaskBuilder.ret)

            member inline this.Bind
                (
                    configurableTaskLike,
                    continuation: _ -> 'a TaskBuilder.Step
                ) : 'a TaskBuilder.Step =
                TaskBuilder.Binder<'a>
                    .GenericAwaitConfigureFalse(configurableTaskLike, continuation)
